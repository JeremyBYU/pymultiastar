...
from .types import PlannerKwargs, ArrayFloatMxNxK, VoxelMeta, GPS, LandingSite
from .helper import (
...
class GeoPlanner(object):
    ...
    def plan_multi_goal(
        ...
        start_cell = convert_voxel_coord(
            project_start, self.voxel_meta
        )  # to cell position
        ...
        for i, (goal_pos, goal_value) in enumerate(projected_goals):
            gc = convert_voxel_coord(goal_pos, self.voxel_meta)
            ...
            if bad_goal:
                ...
                if gc is None:
                    ...
                    gc = get_first_free_cell_up(
                        gc_, self.voxel_meta, self.cost_map, to_cell=False
                    )
                    if gc is None:
        ...
        path_cells, path_cost, meta = self.planner.search_multiple(
        ...
        logger.debug("Path Cost: %s ", path_cost)
        # These are index coordinates! Convert to meters
        path_meters = [
            convert_voxel_coord(cell, self.voxel_meta, to_cell=False)
            for cell in path_cells
        ]

        path_length = get_path_dist(path_meters)
